            ...
            double * clusters)        /* out: [numClusters][numCoords] */
{
	...
	/*for(h=0 ; h<numClusters ; h++){
		printf("Rank %d : rank_newClusterSize[%d]=%d\n",rank,h,rank_newClusterSize[h]);
	}
	*/
...
/*	if(rank==0){
	      for(h=0 ; h<numClusters ; h++){
                printf("Rank %d : newClusterSize[%d]=%d\n",rank,h,newClusterSize[h]);
        }
	}
*/
...
/*       long int ii;	
        if (rank == 0) {
        	printf("Final cluster centers in kmeans.c:\n");
       		 for (ii=0; ii<numClusters; ii++) {
           	 printf("clusters[%ld] = ",ii);
           	 for (j=0; j<numCoords; j++)
                printf("%6.6f ", clusters[ii*numCoords + j]);
            printf("\n");
        }
        }
  */  
	...
	MPI_Allreduce(&rank_delta, &delta, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        ...
        delta /= numObjs;
       // if(rank==0)printf("Delta of rank 0 in loop %d is %lf\n",loop,delta);
        loop++;
        //printf("\r\tcompleted loop %d", loop);
        //fflush(stdout);
    } while (delta > threshold && loop < loop_threshold);
   // printf("I am process %d and I ve just finished the kmeans call\n",rank);    
    timing = wtime() - timing;
