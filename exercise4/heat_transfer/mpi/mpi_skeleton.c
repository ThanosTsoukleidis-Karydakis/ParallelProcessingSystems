...
int main(int argc, char ** argv) {
	...
	/*Make sure u_current and u_previous are
		both initialized*/
       //printf("rank %d arrived here\n",rank);
	int ind_x, ind_y;
/*	for(ind_x=0;ind_x<local[0]+2;ind_x++){
		for(ind_y=0;ind_y<local[1]+2;ind_y++){
			u_current[ind_x][ind_y]=0;
		//	u_previous[ind_x][ind_y]=0;
		}
	}
*/
 ...
 //	printf("rank %d finished\n",rank);
	/*
	local[1]+3: *u_current = index in the first element of the first row of u_current(which is a ghost cell).We need to avoid saving things to ghost cells. 
	*/
//	printf("rank %d before scatter\n",rank);
	
	MPI_Scatterv(*U,scattercounts,scatteroffset,global_block,(*u_current+local[1]+3),1,local_block,0,CART_COMM);	
	//printf("rank %d finished scatter\n",rank);
        for(ind_x=0;ind_x<local[0]+2;ind_x++){
                for(ind_y=0;ind_y<local[1]+2;ind_y++){
                        u_previous[ind_x][ind_y]=u_current[ind_x][ind_y];
                //      u_previous[ind_x][ind_y]=0;
                }
        }
...
*/
   // printf("I am rank %d and my coords are (%d,%d)\n",rank,rank_grid[0],rank_grid[1]);
	...
	bool is_internal(int x, int y){
		return x!=0 && y!=0 && x!=grid[0] && y!=grid[1];
	}
	...
	bool is_boundary_not_padded(int x, int y){
		return !is_internal(x,y) && !is_right_padded(x,y) && !is_bottom_padded(x,y);
	}
