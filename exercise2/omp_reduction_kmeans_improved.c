            ...
            double * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    do {
         ...
         */
	int n;
	#pragma omp parallel for private(n, i, j) default(shared)
	for(n=0 ; n<nthreads ; n++){
		for (i=0; i<numClusters; i++) {
           		for (j=0; j<numCoords; j++)
                		local_newClusters[n][i*numCoords + j] = 0.0;
       			local_newClusterSize[n][i] = 0;
       		}	
	}
        
	#pragma omp parallel for private(i, j, index) default(shared) reduction(+:delta)
        for (i=0; i<numObjs; i++)
        {
	...
	for(t=0 ; t<nthreads ; t++){
		for(index=0 ; index<numClusters ; index++){
			for(j=0 ; j<numCoords ; j++){
				newClusters[index*numCoords + j]+=local_newClusters[t][index*numCoords + j];
			}
			newClusterSize[index]+=local_newClusterSize[t][index];
		}
	}
